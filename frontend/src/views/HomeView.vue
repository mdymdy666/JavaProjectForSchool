...
const visibleProducts = computed(() => activeTab.value === 'hot' ? hotProducts.value : newProducts.value)
...
function endDrag(event: PointerEvent) {
  ...
  if (delta < -72) {
    ...
    const id = Number(slide?.dataset.productId)
    if (id) {
      ...
      <aside class="side-panels">
        ...
        <section class="notice-panel">
          <div class="panel-title compact">
            ...
            <h2>平台公告</h2>
            <button type="button" @click="router.push('/products')">更多</button>
          </div>
          <div v-if="announcements.length" class="notice-list">
            <button v-for="a in announcements.slice(0, 2)" :key="a.id" type="button">
              {{ a.title }}
              <i></i>
            </button>
          </div>
