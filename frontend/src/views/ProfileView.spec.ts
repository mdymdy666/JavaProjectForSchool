...
describe('ProfileView', () => {
  ...
  it('shows the uploaded avatar immediately after profile save', async () => {
    ...
    const input = wrapper.get<HTMLInputElement>('.avatar-upload input')
    ...
    expect(updateProfile).toHaveBeenCalledWith(expect.objectContaining({ avatarUrl: '/uploads/avatar.png' }))
    expect(wrapper.get('.avatar img').attributes('src')).toBe('/uploads/avatar.png')
  })
})
